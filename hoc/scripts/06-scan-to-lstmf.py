...
def filter_lines(lines):
    ...
    for i, (s, e, pk) in enumerate(lines):
        h = e - s
        gap = s - lines[i-1][1] if i > 0 else 0
        if i == 0 and (h > med_h * 1.3 or pk > dense):
            skipped.append(i)
            continue
        if i == last and h > med_h * 1.25:
            skipped.append(i)
            continue
        if h > med_h * 1.7:
            skipped.append(i)
        ...
        if pk < sparse:
            skipped.append(i)
            continue
        if gap > med_h * 2.5 and h < med_h * 0.8:
            skipped.append(i)
            continue
        kept.append((s, e, pk))
    return kept, skipped
...
# ── Warang Citi helpers ──────────────────────────────────────────────────────────
def is_warang_citi(c):
    cp = ord(c)
    return (0x118A0 <= cp <= 0x118FF) or (0xE000 <= cp <= 0xE05F)
...
def main():
    ...
    # Load GT: keep only non-blank lines with Warang Citi chars
    raw_gt   = Path(args.gt).read_text(encoding='utf-8').splitlines()
    gt_lines = [ln.strip() for ln in raw_gt
                if ln.strip() and any(is_warang_citi(c) for c in ln)]
    ...
    if len(kept) > len(gt_lines):
        excess = len(kept) - len(gt_lines)
        print(f"  Auto-trimming {excess} extra detected line(s) from bottom")
        kept = kept[:len(gt_lines)]
