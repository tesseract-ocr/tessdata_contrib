...
# Instead, each Ol Chiki character gets its own equal-width x-slice box.
#
# Output:
...
#
# Prerequisites:
#   Run 01-prep-base.sh first (needs tessdata_best/sat_base.traineddata)
# ═══════════════════════════════════════════════════════════════
set -e
...
[ -f "$TESSDATA_BEST/sat_base.traineddata" ] || {
    echo "ERROR: tessdata_best/sat_base.traineddata not found."
...
def ol_chiki_clusters(text):
    """Split Ol Chiki text into character clusters.
    Ol Chiki signs U+1C78-U+1C7D (category Lm) are modifiers that attach
